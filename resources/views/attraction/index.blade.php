...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
    ...
    <div class="lg:col-span-1">
        <form action="{{ route('attractions.index') }}" method="GET" class="card p-4 sticky top-4">
                ...
                <div class="space-y-2">
                    @foreach($categories as $slug => $name)
                        <div class="flex items-center">
                            <input type="checkbox" id="category-{{ $slug }}" name="categories[]" value="{{ $slug }}" 
                                   class="mr-2" {{ in_array($slug, request('categories', [])) ? 'checked' : '' }}>
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center">
                    <input type="number" name="min_price" placeholder="Min" 
                           ...
                           class="w-full p-2 border border-gray-200 rounded-md mr-2">
                    <span class="text-gray-400">-</span>
                    <input type="number" name="max_price" placeholder="Max" 
                           value="{{ request('max_price') }}"
                           class="w-full p-2 border border-gray-200 rounded-md ml-2">
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="space-y-2">
                    <div class="flex items-center">
                        <input type="checkbox" id="duration-1" name="durations[]" value="short" 
                               class="mr-2" {{ in_array('short', request('durations', [])) ? 'checked' : '' }}>
                        <label for="duration-1" class="text-gray-600">Less than 3 hours</label>
                    </div>
                    <div class="flex items-center">
                        <input type="checkbox" id="duration-2" name="durations[]" value="medium" 
                               class="mr-2" {{ in_array('medium', request('durations', [])) ? 'checked' : '' }}>
                        <label for="duration-2" class="text-gray-600">3-5 hours</label>
                    </div>
                    <div class="flex items-center">
                        <input type="checkbox" id="duration-3" name="durations[]" value="full_day" 
                               class="mr-2" {{ in_array('full_day', request('durations', [])) ? 'checked' : '' }}>
                        <label for="duration-3" class="text-gray-600">Full day</label>
                    </div>
                    <div class="flex items-center">
                        <input type="checkbox" id="duration-4" name="durations[]" value="multi_day" 
                               class="mr-2" {{ in_array('multi_day', request('durations', [])) ? 'checked' : '' }}>
            ...
            @if(request()->has('categories') || request()->has('min_price') || request()->has('max_price') || request()->has('durations'))
                <a href="{{ route('attractions.index') }}" class="text-center block mt-4 text-primary hover:underline">Clear Filters</a>
            @endif
        </form>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            @empty
                <div class="col-span-3 text-center py-12">
                    ...
                    <p class="text-gray-500 mb-6">There are currently no attractions available with the selected filters. Please try different filters or check back later.</p>
                    @if(request()->has('categories') || request()->has('min_price') || request()->has('max_price') || request()->has('durations'))
                        <a href="{{ route('attractions.index') }}" class="btn-primary">Clear Filters</a>
                    @endif
                </div>
            @endforelse
        </div>
